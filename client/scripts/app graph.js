...
var cubeNation = [];
for (var x = 0; x < 4; x++) {
	var cubeDimensions = Math.floor(Math.random() * 250 + 50);
...
var meshedCubes = cubeNation.map(function meshAll(item) {
	//item.applyMatrix(new THREE.Matrix4().makeTranslation(0, 50 / 2, 0));
	if(colorCount > 5) {
		colorCount = 0;
	}
...
meshedCubes.forEach(function addToScene(cubic) {
	scene.add(cubic);
	//cubic.position.set(objPositions.x, objPositions.y + cubic.geometry.parameters.height / 2, objPositions.z);
	objPositions.x += 60;
	// objPositions.y += 100;
});
...
function cubeResize(cubes) {
	cubes.forEach(function(cube, idx) {		
		cube.scale.y += idx % 2 === 0 ? 0.001 : 0; // cube.scale.y -= 0.001;
		if (idx % 4 === 0) {
			console.log(cube.geometry.parameters.height * cube.scale.y);
		}
		cube.position.set(cube.position.x, objPositions.y + (cube.geometry.parameters.height * cube.scale.y) / 2 - cube.scale.y, objPositions.z);
	});
}
